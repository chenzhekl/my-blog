...
要取得更好地结果，我们可以使用机器学习的方法，把 N 张手写数字的图片\\( \{\mathbf{x}\_1, \dots, \mathbf{x}\_N\} \\) 作为 _训练集_ ，来调整一个适应性模型的参数。每张图片对应的数字，通常通过手工标注的方式预先知晓。我们可以图片把对应的数字（即类别）以 \*目标向量\_ \\( \mathbf{t} \\) 表达。之后会详细讨论使用向量表达类别的方法。注意，每张图片 \\( \mathbf{x} \\) 对应一个向量 \\( \mathbf{t} \\)。
...
其中 \\( M \\) 是多项式的 _阶数_ ，\\( x^j \\) 表示 \\( x \\) 的 \\( j \\) 次方。多项式系数 \\( w*0, \dots, w_M \\) 合起来由向量 \\( \mathbf{w} \\) 表示。注意，虽然多项式函数 \\( y(x, \mathbf{w}) \\) 是 \\( x \\) 的非线性函数，但它是系数 \\( \mathbf{w} \\) 的线性函数。像多项式这样对于未知参数是线性的函数，它们有重要的性质，被称作 *线性模型\_ ，我们会在第 3 和 4 章详细讨论。
...
{{< figure src="/posts/2018/06/14/figure1.3.png" title="图 1.3" caption="误差函数定义为每个数据点和函数 \\( y(x, \mathbf{w}) \\) 的偏差（显示为为绿色竖线）的平方和（的一半）。" width="100%" >}}
...
其中处以 \\( N \\) 允许我们同等的比较不同大小的数据集，平方根保证了 \\( E\_\mathrm{RMS} \\) 与目标变量 \\( t \\) 拥有相同尺度（与单位）。不同 \\( M \\) 值下训练与测试集的均方根误差见图 1.5。测试集误差表示对新观测 \\( x \\) 的预测 \\( t \\) 的准确程度。从图 1.5 可以看出，较小的 \\( M \\) 值会造成较大的测试误差，这是因为对应的多项式灵活度不足，不足以捕捉函数 \\( \sin{(2\pi x)} \\) 的震荡。\\( M \\) 的值在 \\( 3 \le M \le 8 \\) 间时给出较小的测试集误差，从 \\( M = 3 \\) 可以看出，此时多项式也足够表示生成函数 \\( \sin{(2\pi x)} \\)。
...
其中 \\( {\lVert \mathbf{w} \rVert}^2 = \mathbf{w}^\mathrm{T}\mathbf{w} = w*0^2 + w_1^2 + \dots + w_M^2 \\)，系数 \\( \lambda \\) 控制正则化项相对平方和误差项的重要程度。注意，大家经常在正则化项中省略\\( w_0 \\)，因为若将其包含在内，会使得结果受到目标变量原点选择的影响（Hastic \_et al.*, 2001）。另一种解决方案是包含 \\( w*0 \\)，但为其单独设置正则化项系数（我们会在 5.5.1 节详细讨论这个话题）。误差函数 \\( \eqref{eq:sum-square-penalty} \\) 同样可以解析地获得最小值。在统计学文献中，这种技术被称作 *缩减* 方法，因为减小了系数项的值。当使用二次正则化项时，这种曲线拟合方法被称作 *岭回归* （Hoerl and Kennard, 1970）。在神经网络中，同样的方法被称作 *权值衰减\_ 。
...
首先，我们定义某事件的概率为如下极限：当实验次数趋向无穷大时，事件发生的次数占总实验次数的比值。因此选取红盒子的概率是 \\( \frac{4}{10} \\)，选取蓝盒子的概率为 \\( \frac{6}{10} \\)。分别记为：\\( p(B = r) = \frac{4}{10} \\)，\\( p(B = b) = \frac{6}{10} \\)。注意，依定义概率值必须在 \\( [0, 1] \\) 之间。另外，如果一组事件互斥，且它们包含随机实验的所有可能结果（比如在上述例子中，盒子只能是红色或者蓝色），这些事件的概率之和必然等于 \\( 1 \\)。

现在我们可以提问：“拿出一个苹果的概率是多少？”，或是“假设拿出的是一个橘子，我们选择的盒子是蓝色的概率是多少？”。在掌握概率论中两条基本原则（ _加法原则_ 和 _乘法原则_ ）后，我们就可以回答诸如上述这些、亦或是和模式识别有关的更复杂的问题。让我们先来了解这些原则。

为了导出这些规则，我们考虑一个更一般的例子（图 1.10）。有两个随机变量 \\( X \\)，\\( Y \\)，\\( X \\) 可以取 \\( x*i \\) 中的任何值，\\( i = 1, \dots, M \\)。\\( Y \\) 可以取 \\( y_j \\) 中的任何值，\\( j = 1, \dots, L \\)。同时采样 \\( X \\) 和 \\( Y \\)，重复 \\( N \\) 次，将其中 \\( X = x_i, Y = y_i \\) 的次数记为 \\( n*{ij} \\)。另外，将其中 \\( X \\) 取 \\( x_i \\) 的次数（无论 \\( Y \\) 为何值）记为 \\( c_i \\)，\\( Y \\) 取 \\( y_j \\) 的次数记为 \\( r_i \\)。

{{< figure src="/posts/2018/06/14/figure1.10.png" title="图 1.10" caption="我们考虑两个随机变量 \\( X \\)，\\( Y \\)。\\( X \\) 取值 \\( \{ x_i \}, i = 1, \dots, M \\)，\\( Y \\) 取值 \\( \{ y_i \}, i = 1, \dots, L \\)。图中我们预设 \\( M = 5 \\)，\\( L = 3 \\)。重复进行 \\( N \\) 次实验，将 \\( X = x_i, Y = y_i \\) 的次数记为 \\( n_{ij} \\)，即图中矩阵的一个单元对应的点的数量。列 \\( i \\) 包含的点的总数量（对应 \\( X = x_i \\)）为 \\( c_i \\)；行 \\( j \\) 包含的点的总数量（对应 \\( Y = y_j \\)）为 \\( r_j \\)。" width="100%" >}}

\\( X \\) 取 \\( x*i \\) 且 \\( Y \\) 取 \\( y_j \\) 的概率写成 \\( p(X = x_i, Y = y_j) \\)，被称作 \\( X = x_i \\) 且 \\( Y = y_j \\) 的 *联合\_ 概率分布值。这可以通过将单元 \\( i, j \\) 中点的数量处以所有单元总的点数获得，因此
...
这里我们假设 \\( N \rightarrow \infty\\)。类似的，\\( Y \\) 取任何值且 \\( X \\) 取 \\( x_i \\) 的概率写成 \\( p(X = x_i) \\)，定义为落在列 \\( i \\) 中的点数与总点数的比值，因此
...
如图 1.10，列 \\( i \\) 的样本数是该列所有样本数之和，因此 \\( c*i = \sum_j{n*{ij}}\\)，结合 \\( \eqref{eq:1.5} \\) 与 \\( \eqref{eq:1.6} \\) 可得
...
这就是概率的 _加法原则_ ，因为是通过求和（边际化）其他变量获得，\\( p(X = x*i) \\)有时被称作 *边际\_ 概率分布。

如果我们只考虑 \\( X = x*i \\) 的样本，那么在其中满足 \\( Y = y_j \\) 的样本比例写成 \\( p(Y = y_j | X = x_i) \\)，称作 \\( X = x_i \\) 时，\\( Y = y_j \\) 的 *条件\_ 概率分布。可以通过计算落在单元 \\( i, j \\) 的点数与列 \\( i \\) 的总点数的比值获得
...
结合 \\( \eqref{eq:1.5} \\)，\\( \eqref{eq:1.6} \\) 和 \\( \eqref{eq:1.8} \\)，我们可以推导出如下关系
...
到目前为止，我们尽可能小心的区分随机变量（例如盒子 \\( B \\)）和它们的取值（比如选择红盒子是取 \\( r \\)），合起来， \\( B \\) 取 \\( r \\) 的概率写成 \\( p(B = r) \\)。虽然这么作可以避免误解，但会使公式变得复杂，而大说数时候没必要拘泥于形式。因此在上下文清晰时，我们把随机变量 \\( B \\) 的分布写成 \\( p(B) \\)，变量取 \\( r \\) 时的概率记成 \\( p(r) \\)。

采用简记法，这两条概率论的基本原则可以写成如下形式。

<div>
\begin{align}
\textbf{加法原则} & \qquad p(X) = \sum_Y{p(X, Y)}\\
\textbf{乘法原则} & \qquad p(X, Y) = P(Y | X)p(X)
\end{align}
</div>

其中 \\( p(X, Y) \\) 是一个联合概率分布， 读作“\\( X \\) _和_ \\( Y \\) 的概率“。类似的，\\( p(Y | X) \\) 是条件概率分布，读作”_给定_ \\( X \\) 时，\\( Y \\) 的概率“。\\( p(X) \\) 为边际概率分布，简单读作”\\( X \\) 的概率“。这两条简单原则构成了本书中概率原理的基础。

结合乘法原则和对称性 \\( p(X, Y) = p(Y, X) \\)，可以立刻导出如下条件概率
...
这就是在机器学习和模式识别中占据中心位置的 _贝叶斯定理_ 。使用加法原则，贝叶斯定理的分母可以改写成分子中出现过的项之和
...
我们可以把贝叶斯定理的分母看成标准化常数，保证了 \\( \eqref{eq:1.12} \\) 左侧条件概率对所有 \\( Y \\) 求和等于 \\( 1 \\)。

在图 1.11 中，我们用一个包含两个随机变量的联合概率分布，展示边际概率分布和条件概率分布的概念。左上角的图展示了从联合概率分布中采样得到的 \\( 60 \\) 个样本。右上角的直方图展示了 \\( Y \\) 的两个可能取值分别在所有样本中占的比例。依概率的定义，当 \\( N \rightarrow \infty \\) 时，这些比例等于对应的概率 \\( p(Y) \\)。我们可以把直方图看作用有限的样本点描述概率分布的一种方式。用数据估计概率是统计模式识别的核心，会在本书中详细介绍。图 1.11 剩下的两幅对应 \\( p(X) \\) 和 \\( p(X | Y = 1)\\) 的直方图估计。

{{< figure src="/posts/2018/06/14/figure1.11.png" title="图 1.11" caption="二元联合分布的图示，\\( X \\) 有 \\( 9 \\) 个可能的取值，\\( Y \\) 有 \\( 2 \\) 个。左上角的图展示了从联合概率分布中采样得到的 \\( 60 \\) 个样本。剩下的直方图分别是 \\( p(X) \\)、\\( p(Y) \\) 的边际概率分布，以及右下角的条件概率分布 \\( p(X | Y = 1)\\)。" width="100%" >}}

现在回到两箱水果的例子。暂时，我们再次显式区分随机变量与其取值。已经知道选取红色和蓝色盒子的概率分别是
...
注意，它们满足 \\( p(B = r) + p(B = b) = 1 \\)。

现在假设我们随机选取了蓝色盒子，那么拿到苹果的概率是苹果在蓝盒子中的比例，即 \\( \frac{3}{4} \\)，因此 \\( p(F = a | B = b) = \frac{3}{4} \\)。事实上，我们可以写出所有盒子、水果组合的条件概率分布

<div>
\begin{align}
p(F = a | B = r) & = \frac{1}{4} \label{eq:1.16} \\
p(F = o | B = r) & = \frac{3}{4} \\
p(F = a | B = b) & = \frac{3}{4} \\
p(F = o | B = b) & = \frac{1}{4} \label{eq:1.19}
\end{align}
</div>
...
类似的应用加法原则，\\( p(F = o) = 1 - \frac{11}{20} = \frac{9}{20} \\)。

假设我们只被告知取到了一个橘子，想要知道是从哪个盒子来的。这需要我们计算给定水果时盒子的条件概率，然而式 \\( \eqref{eq:1.16} - \eqref{eq:1.19} \\) 只给出了给定盒子时水果的条件概率。我们可以利用贝叶斯定理解决这个逆条件概率的问题
...
依加法原则，\\( p(B = b | F = o) = 1 - \frac{2}{3} = \frac{1}{3} \\)。

从这个例子中，我们可以给出贝叶斯定理的一种解读。如果在告知取到的水果前，被问是从哪个盒子取出的，这时我们手头最完整的信息是 \\( p(B) \\)。我们称它为 _先验概率_ ，因为它是在观测到水果的种类 _前_ 就知道的。一旦被告知了取得的水果是橘子，我们就可以利用贝叶斯定理计算 \\( p(B | F) \\)，称为 _后验概率_ ，因为它是在观察到 \\( F \\) _后_ 获得的。在这个例子中，选择红盒子的先验概率是 \\( \frac{4}{10} \\)，所以相比之下我们更有可能选择蓝盒子。然而在知道取得的水果是橘子后，我们发现选择红盒子的后验概率是 \\( \frac{2}{3} \\)，所以我们更可能是从红盒子里取得的水果。这个结果符合直觉，因为红盒子里橘子的比例远高于蓝盒子中，所以观测到水果是橘子这件事，提供了足够的证据偏向红盒子。事实上，这个证据甚至足够强过先验知识，是我们相比蓝盒子更可能选择红盒子。

最后，我们注意到如果两个变量的联合概率分布可以被分解成乘积的形式，即 \\( p(X, Y) = p(X)p(Y) \\)，就说 \\( X \\) 和 \\( Y \\) 是独立的。结合乘法原则，可以得到 \\( p(Y | X) = p(Y) \\)，即给定 \\( X \\) 时 \\( Y \\) 的条件概率分布确实和 \\( X \\) 的取值无关。比如，在水果的例子中，如果每个盒子装有相同比例的苹果和橘子，那么 \\( p(F | B) = p(F）\\)，也就是说，取到苹果的概率和所选的盒子无关。

### 概率密度

除了考虑离散事件的概率外，我们也希望考虑定义在连续随机变量生的概率分布。这里给出非正式的介绍。如果当 \\( \delta x \rightarrow 0 \\) 时，实数变量 \\( x \\) 落在区间 \\( (x, x + \delta x) \\) 的概率为 \\( p(x) \delta x \\) ，那么 \\( p(x) \\) 就称为 \\( x \\) 的 _概率密度_ （见图 1.12）[^2]。\\( x \\) 落在区间 \\( (a, b) \\) 的概率为

[^2]: 译注：可以类比于物体的密度（概率密度）与质量（概率）。
...
{{< figure src="/posts/2018/06/14/figure1.12.png" title="图 1.12" caption="离散变量的概率通过概率密度 \\( p(x) \\) 拓展到连续变量。具体如下，取极限 \\( \delta x \rightarrow 0 \\)，如果对定义域内的任意 \\( x \\)，其落在区间 \\( (x, x + \delta x) \\) 的概率为 \\( p(x) \delta x \\)，则 \\( p(x) \\) 称为其概率密度。概率密度也可以看成累积概率分布 \\( P(x) \\) 的导数。" width="100%" >}}

因为概率的非负性，以及 \\( x \\) 必然落于实轴某处，概率密度必须满足两个条件
...
由于存在雅可比项，对随机变量作非线性变换，对应的概率密度的改变不同于一个简单函数。举例来说，对简单函数作变量代换 \\( x = g(y) \\)，那么函数 \\( f(x) \\) 变成 \\( \hat{f}(y) = f(g(y))\\)。现在考虑概率密度 \\( p_x(x) \\)，进行同样的变量代换，得到 \\( y \\) 的概率密度 \\( p_y(y) \\)，这里不同的下标代表 \\( p_x(x) \\) 和 \\( p_y(y) \\) 是不同的概率密度。对极小的 \\( \delta x \\)，随机变量落在 \\( (x, x + \delta x)\\) 的概率等价转换为落在 \\( (y, y + \delta y) \\) 的概率，即 \\( p_x(x) \delta x \approx p_y(y) \delta y \\)，因此
