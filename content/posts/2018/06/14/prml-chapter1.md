...
为了导出这些规则，我们考虑一个更一般的例子（图 1.10）。有两个随机变量 `$ X $`，`$ Y $`，`$ X $` 可以取 `$ x_i $` 中的任何值，`$ i = 1, \dots, M $`。`$ Y $` 可以取 `$ y_j $` 中的任何值，`$ j = 1, \dots, L $`。同时采样 `$ X $` 和 `$ Y $`，重复 `$ N $` 次，将其中 `$ X = x_i, Y = y_i $` 的次数记为 `$ n_{ij} $`。另外，将其中 `$ X $` 取 `$ x_i $` 的次数（无论 `$ Y $` 为何值）记为 `$ c_i $`，`$ Y $` 取 `$ y_j $` 的次数记为 `$ r_i $`。
...
`$ X $` 取 `$ x_i $` 且 `$ Y $` 取 `$ y_j $` 的概率写成 `$ p(X = x_i, Y = y_j) $`，被称作 `$ X = x_i $` 且 `$ Y = y_j $` 的 _联合_ 概率分布值。这可以通过将单元 `$ i, j $` 中点的数量处以所有单元总的点数获得，因此
...
如图 1.10，列 `$ i $` 的样本数是该列所有样本数之和，因此 `$ c_i = \sum_j{n_{ij}}$`，结合 `$ \eqref{eq:1.5} $` 与 `$ \eqref{eq:1.6} $` 可得
...
这就是概率的 _加法原则_ ，因为是通过求和（边际化）其他变量获得，`$ p(X = x_i) $`有时被称作 _边际_ 概率分布。

如果我们只考虑 `$ X = x_i $` 的样本，那么在其中满足 `$ Y = y_j $` 的样本比例写成 `$ p(Y = y_j | X = x_i) $`，称作 `$ X = x_i $` 时，`$ Y = y_j $` 的 _条件_ 概率分布。可以通过计算落在单元 `$ i, j $` 的点数与列 `$ i $` 的总点数的比值获得
